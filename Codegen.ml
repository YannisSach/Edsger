...
and codegen_declaration x = match x with
  ...
  | Ast.Variable_dec(ty, decs) ->
     ...
     let value = (match ty with
                  ...
                  |_ -> raise (Type_error "problem with value allocation")) in (* NOT SURE IF WE WANT THE POINTER TO BE NULL AT FIRST *)
  
     let _ = List.map (fun dec -> match dec with
                                  | Ast.Simple_declarator(name) ->
                                     ...
                                     (* let _ = begin *)
                                     (*    try *)
                                     (*            let old_value = Hashtbl.find named_values name in *)
                                     (*            old_bindings := (name, old_value) :: !old_bindings; *)
                                     (*          with Not_found -> () *)
                                     (*        end in *) 
                                  ...
                                  | Ast.Complex_declarator(name, exp) ->
                                     ...
                                     let alloca = build_array_malloc typos leng name builder in
                                     Hashtbl.add named_values name alloca;
                      (* HAVE TO CHECK IT AGAIN *)
                      ) decs; in
     const_null null_type;
  ...
  | Ast.Function_def(ty, name, parameters, decls, stms) ->
     ...
     let _ = Array.iteri(fun i a ->
                 ...
                 | Ast.By_ref_param(ty, name) ->
                    (* let typ = type_to_lltype ty in *)
                    (* let point = pointer_type typ in *)
                    (* let alloca = build_alloca point name builder in *)
                    (* ignore(build_store a alloca builder); *)
                    set_value_name name a;
                    Hashtbl.add named_values name a)
                        (params the_fun)in
...
and codegen_statement st = match st with
  ...
  | Ast.If_stmt(cond, stm) ->
     ...
     let bef_bb_val = value_of_block bef_bb in
     ...
     let then_val = codegen_statement stm in  (* we create the code for the then statement *)
  ...
  | Ast.If_else_stmt(cond, stm1, stm2) ->
     ...
     let then_val = codegen_statement stm1 in
     ...
     let else_val = codegen_statement stm2 in
